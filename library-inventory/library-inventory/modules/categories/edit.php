...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_csrf($token)) {
        ...
        if (empty($namaKategori)) {
            ...
            $stmtCek = $db->prepare("SELECT id FROM categories WHERE nama_kategori = ? AND id != ?");
            $stmtCek->execute([$namaKategori, $id]);
            if ($stmtCek->fetch()) {
                $error = 'Kategori dengan nama tersebut sudah ada.';
            } else {
                $stmtUpdate = $db->prepare("UPDATE categories SET nama_kategori = ?, deskripsi = ? WHERE id = ?");
                $stmtUpdate->execute([$namaKategori, $deskripsi, $id]);

                log_activity($db, 'EDIT_KATEGORI', "Memperbarui kategori ID {$id}: '{$namaKategori}'");
                set_flash('success', "Kategori '{$namaKategori}' berhasil diperbarui!");
                redirect('modules/categories/index.php');
            }
        }
    }
    ...
    <div class="col-12">
        <div class="d-flex align-items-center justify-content-between">
            <div>
                ...
                <p class="text-muted small mb-0">Ubah informasi nama dan deskripsi kategori buku.</p>
            </div>
    ...
    <div class="col-lg-6 mx-auto">
        <div class="card shadow-sm border-0">
            <div class="card-header bg-white py-3">
                <h5 class="card-title fw-bold mb-0 text-primary">Form Edit Kategori</h5>
            </div>
            <div class="card-body p-4">
                <form action="<?= base_url('modules/categories/edit.php?id=' . $id) ?>" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="nama_kategori" name="nama_kategori" 
                               value="<?= htmlspecialchars($namaKategori) ?>" required autofocus>
                    </div>
